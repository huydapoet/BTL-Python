...
# Hàm vẽ Elbow plot
def elbow_plot(X_scaled_data, title, output_path, file_name, kmin=2, kmax=10):
    distortions = []
...
def silhouette_multi_plot(X_scaled_data, title, output_path, file_name, k_min=2, k_max=5):
    ...
    plt.savefig(out_path)
    plt.close()
    print(f"Saved Silhouette Plots to {out_path}")
...
def plot_pca_2d(df, X_scaled_data, title, output_path, file_name, cluster_col='Cluster'):
    ...
    plt.savefig(out_path)
    plt.close()
    print(f"Saved PCA 2D to: {out_path}")
...
def plot_pca_3d(df, X_scaled_data, title, output_path, file_name, cluster_col='Cluster'):
    ...
    plt.savefig(out_path)
    plt.close()
    print(f"Saved PCA 3D to: {out_path}")
...
elbow_plot(X_gk_scaled, 'The Elbow Plot showing the optimal k for GK',GK_OUTPUT, 'elbow_gk.png', kmin=2, kmax=6)
...
kmeans_gk_opt = KMeans(n_clusters=3, random_state=42, n_init=10)
